		...
		$( 'a.mw-lingo-term' ).each( function () { // eslint-disable-line no-jquery/no-global-selector
			...
			$( this ).on( 'focus focusout', function ( e ) {
				if ( e.type === 'focus' ) {
					...
					const relatedTarget = $( e.relatedTarget ), // eslint-disable-line no-jquery/variable-pattern
		...
		$( document ).on( 'keydown', ( e ) => {
			if ( e.key === 'Escape' && lastFocus ) {
				...
				if ( tooltipDefinitions.length > 0 ) {
					const focusableElements = tooltipDefinitions.find( '.mw-lingo-definition-link > a' ),
